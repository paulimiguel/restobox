...
function notesWithBullets(value: string) {
	const normalized = value.replace(/\r\n?/g, '\n');
	if (!normalized) return '';
	return normalized.split('\n').map((line) => {
		const content = line.replace(/^\s*•\s?/, '').trimStart();
		return `${NOTES_BULLET}${content}`;
	}).join('\n');
}
...
function setupClearableFields() {
	...
	controls.forEach((control) => {
		...
		if (['cuisine', 'establishment-type', 'service-type'].includes(control.id)) return;
			...
			<article class="restaurant-card${printSelectedIds.has(restaurant.id) ? ' print-selected' : ''}" data-restaurant-id="${restaurant.id}">
				<div class="restaurant-card-media">
					...
					<div class="restaurant-card-top-actions" aria-label="Acciones del restaurante">
						...
						<button class="checked-button${restaurant.checked ? ' active' : ''}" type="button" data-checked="${restaurant.id}" aria-pressed="${Boolean(restaurant.checked)}" aria-label="${restaurant.checked ? 'Marcar como no chequeado' : 'Marcar como chequeado'}" title="Checked"><span class="checked-icon" aria-hidden="true"></span></button>
